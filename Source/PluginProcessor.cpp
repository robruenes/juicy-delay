...
DelayAudioProcessor::DelayAudioProcessor()
    ...
    gainParam_ = dynamic_cast<juce::AudioParameterFloat*>(
        apvts_.getParameter(gainParamID.getParamID()));
}
...
void DelayAudioProcessor::processBlock(
    ...
    float gainInDecibels = gainParam_->get();
...
void DelayAudioProcessor::setStateInformation(const void* data,
    ...
    if (xml.get() != nullptr && xml->hasTagName(apvts_.state.getType())) {
...
ParameterLayout DelayAudioProcessor::createParameterLayout() {
    ParameterLayout layout;
    layout.add(std::make_unique<juce::AudioParameterFloat>(
        gainParamID, "Output Gain",
        juce::NormalisableRange<float>{-12.0f, 12.0f}, /*default=*/0.0f));
    return layout;
}
